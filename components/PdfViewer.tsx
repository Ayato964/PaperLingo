...
export const PdfViewer: React.FC<PdfViewerProps> = ({ pdfDoc, pageNumber, scale = 1.5 }) => {
  ...
  if (!pdfDoc) return <div className="bg-slate-200 dark:bg-slate-800 h-full animate-pulse transition-colors"></div>;

  return (
    <div className="w-full overflow-auto flex justify-center p-1">
      <canvas
        ref={canvasRef}
        className="shadow-lg max-w-full h-auto object-contain bg-white dark:bg-slate-200"
      />
    </div>
  );
};