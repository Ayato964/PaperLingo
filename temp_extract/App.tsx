...
import { Book, UploadCloud, BrainCircuit, CheckCircle2, Lock, ChevronRight, FileText, Image as ImageIcon, Settings, AlertCircle, ZoomIn, ZoomOut } from 'lucide-react';
...
import { AppStatus, Section, GlossaryItem } from './types';
...
const App: React.FC = () => {
  ...
  const loadSectionData = async (sectionId: number) => {
    ...
    try {
      ...
      const data = await generateGlossaryForSection(section.content, apiKey);
      setSectionGlossary(data.glossary);
    } catch (err) {
      console.error("Glossary generation failed", err);
      setError("単語帳の生成に失敗しました。");
    } finally {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      const skeletalSections = await analyzeStructure(fullText, apiKey);
      
      // 4. Construct complete sections locally using pageTexts
      // This avoids asking AI to regenerate text it just read, speeding up the process significantly.
      const completeSections: Section[] = skeletalSections.map((s, index) => {
        // Extract content from pageTexts based on start/end page (1-based index)
        ...
        return {
          ...
          endPage: s.endPage,
          content: content,
          summary: "学習を開始して内容を確認しましょう。" // Placeholder for MVP
        };
      });
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-sky-200 selection:text-sky-900">
      ...
      <SelectionPopup apiKey={apiKey} />

      {/* Header */}
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm h-16 flex items-center justify-between px-6">
        <div className="flex items-center gap-2">
          <div className="bg-sky-600 p-1.5 rounded-lg text-white">
            <Book className="w-5 h-5" />
          </div>
          <span className="font-bold text-xl text-slate-800 hidden sm:inline">PaperLingo</span>
        </div>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${isCurrent ? 'bg-sky-600 text-white ring-2 ring-sky-200' : 
                      ...
                      isLocked ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}
                  `}
                >
        ...
        <button onClick={() => setShowApiKeyModal(true)} className="text-slate-400 hover:text-slate-600">
      ...
      <main className="max-w-[1600px] mx-auto p-4 sm:p-6 h-[calc(100vh-4rem)]">
        {status === AppStatus.IDLE || status === AppStatus.PARSING_PDF || status === AppStatus.ANALYZING_STRUCTURE || status === AppStatus.ERROR ? (
          <div className="max-w-xl mx-auto mt-20 text-center">
             <div className="mb-8">
                <h1 className="text-4xl font-bold text-slate-900 mb-4">論文を、マスターしよう。</h1>
                <p className="text-lg text-slate-600">
                  PaperLingoは、AIを活用して論文の構造を解析し、<br />
                  単語学習から翻訳演習までをサポートする学習プラットフォームです。
                </p>
             </div>
             ...
             {status === AppStatus.ANALYZING_STRUCTURE && (
                <div className="mt-6 flex flex-col items-center text-sky-600 animate-pulse">
                  <BrainCircuit className="w-8 h-8 mb-2" />
                  <p>AIが論文の構成をスキャンしています...</p>
                </div>
             )}

             {error && (
              <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3 text-red-700 text-left">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
            ...
            <div className="flex flex-col h-full bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="flex border-b border-slate-100 relative">
                <button 
                  ...
                  className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === 'text' ? 'text-sky-600 border-b-2 border-sky-600 bg-sky-50/50' : 'text-slate-500 hover:text-slate-700'}`}
                >
                ...
                <button 
                  ...
                  className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === 'visual' ? 'text-sky-600 border-b-2 border-sky-600 bg-sky-50/50' : 'text-slate-500 hover:text-slate-700'}`}
                >
                ...
                {activeTab === 'visual' && (
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-0.5 shadow-sm">
                    <button onClick={handleZoomOut} className="p-1 hover:bg-slate-100 rounded text-slate-600" title="縮小">
                      <ZoomOut className="w-4 h-4" />
                    </button>
                    <span className="text-xs w-12 text-center font-mono text-slate-500">{Math.round(visualScale * 100)}%</span>
                    <button onClick={handleZoomIn} className="p-1 hover:bg-slate-100 rounded text-slate-600" title="拡大">
              ...
              <div className="flex-1 relative overflow-hidden">
                {activeTab === 'text' ? (
                   <div className="h-full overflow-y-auto p-6 font-serif text-lg leading-loose text-slate-800 selection:bg-yellow-200 cursor-text">
                     ...
                     <p className="mt-8 text-center text-slate-400 text-sm italic">-- Section End --</p>
                   </div>
                ) : (
                   <div className="h-full bg-slate-800 overflow-y-auto p-4 text-center">
                      ...
                      {Array.from({ length: currentSection.endPage - currentSection.startPage + 1 }, (_, i) => currentSection.startPage + i).map(pageNum => (
                        <div key={pageNum} className="mb-4">
                           ...
                           <p className="text-slate-400 text-xs mt-2">Page {pageNum}</p>
                        </div>
                      ))}
                   </div>
                )}
            ...
            <div className="flex flex-col h-full gap-6">
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-slate-800">Section {currentSection.id + 1} Tasks</h3>
                  <p className="text-sm text-slate-500">
                    {learningStep === 'vocab' ? 'Step 1: 重要単語を覚える' : learningStep === 'translation' ? 'Step 2: 理解度チェック' : 'セクション完了！'}
                  </p>
                </div>
                <div className="flex items-center gap-1">
                  <div className={`h-2 w-8 rounded-full ${learningStep === 'vocab' ? 'bg-sky-500' : 'bg-green-500'}`}></div>
                  <div className={`h-2 w-8 rounded-full ${learningStep === 'vocab' ? 'bg-slate-200' : learningStep === 'translation' ? 'bg-sky-500' : 'bg-green-500'}`}></div>
                </div>
              </div>
                ...
                <div className="p-6 flex-1 overflow-y-auto">
                  {isGlossaryLoading ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center p-8">
                      ...
                      <h2 className="text-2xl font-bold text-slate-800 mb-2">Excellent Work!</h2>
                      <p className="text-slate-600 mb-8">
                        このセクションの内容をマスターしました。<br/>
                        次のセクションに進んで学習を続けましょう。
                      </p>
                      {currentSectionId < sections.length - 1 ? (
                        <button 
                          onClick={handleNextSection}
                          className="bg-sky-600 hover:bg-sky-700 text-white px-8 py-4 rounded-xl font-bold shadow-lg hover:scale-105 transition-all flex items-center gap-2"
                        >
                          次のセクションへ <ChevronRight className="w-5 h-5" />
                        </button>
                      ) : (
                        <p className="text-sky-600 font-bold">論文全体の学習が完了しました！お疲れ様でした！</p>
                      )}
                    </div>
                  )}
                </div>
